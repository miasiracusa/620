...
edgeList <- yelp_network %>% select(businessID, userID, weight, size)
...
g <- add.vertices(g, 
                  ...
                  color = 'red',
                  attr=list(name=paste0('B',business_nodes),type=rep(TRUE)))
## User Nodes (white)
g <- add.vertices(g,
                  nv=length(user_nodes),
                  color = 'white',
                  attr=list(name=paste0('U',user_nodes), type=rep(FALSE)))

## Set Node Attribute based on T/F type
V(g)$shape <- ifelse(V(g)$type, "circle", "square")

## Set size of nodes based on degree
#deg <- degree(g, mode="all")
#V(g)$size <- ifelse(V(g)$type, V(g)$size, 10)

# CREATE EDGES
## vectorize edge list
edgeListVec <- as.vector(t(as.matrix(data.frame(Business=paste0('B',edgeList$businessID),
                             User=paste0('U',edgeList$userID)))))
## add edges to graph
...
Is.Weighted <- is.weighted(g)
Is.Bipartite <- is.bipartite(g)

rbind(Is.Weighted, Is.Bipartite) %>% kable() %>% kable_styling()
```


### Visualize Network 

```{r view-network, echo=F}
plot.igraph(g, 
...
```{r}
degree(g, v = V(g)$type==FALSE, mode='all')


```

```{r}
edges <- yelp_network %>% select(businessID, userID, weight) %>% spread(businessID, weight)
rownames(edges)<-edges$userID 
links <-edges %>% select(-userID)
