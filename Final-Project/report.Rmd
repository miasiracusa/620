...
## Locality Network  {.tabset .tabset-fade .tabset-pills}
...
map(database = "county", regions = "arizona",col="tomato",  border="gray10", fill=TRUE)
points(x=yelp_network$longitude, y=yelp_network$latitude, pch=19, cex=yelp_network$size/50, col="orange")

```

```{r include=F, eval=F}
lookup_code("Arizona","Scottsdale")

yelp %>% select(city) %>% group_by(city) %>% tally() %>% arrange(desc(n))
```
